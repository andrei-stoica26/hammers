...
finding and removing rare genes, performing multiple testing correction, 
...
the `dimPlot` function from `scLang`. `genesDimPlot` uses `geneCenters` 
internally:
...
The `distributionPlot` function plots the frequencies of elements from two 
categorical columns:
...
It can also plot relative percentages instead of raw counts:
