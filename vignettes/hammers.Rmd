...
on `Seurat` and `SingleCellExperiment` objects. These are intended to help 
developers build tools compatible with both types of input.

Additionally, `hammers` includes simple tools to address frequent tasks in 
scRNA-seq data analysis, such as retrieving aggregate gene statistics, 
finding and removing rare genes, performing multiple testing correction, and 
calculating silhouette and cluster-normalized silhouette.
...
`scExpMat` can also take a matrix as an argument. This option is useful when building
...
**Note**: Just like `scCol`, the `metadataDF` and `metadataNames` functions have setters
which allow users to alter the value of the stored objects.
...
By default, `repAnalysis` calls `mtCorrectDF` with the mtMethod parameter set
to `by` (Benjamini-Yekutieli). Other options are `bf` (Bonferroni) and `bh`
(Benjamini-Hochberg):
...
More generally, `pvalRiverPlot` can create an alluvial plot for any data frame
